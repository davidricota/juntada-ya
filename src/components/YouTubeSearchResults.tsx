...
const YouTubeSearchResults: React.FC<YouTubeSearchResultsProps> = ({ results, onSongSelected, showAddButton = true }) => {
        ...
        <ScrollArea className="w-full max-h-96 rounded-lg">
          <ul className="space-y-3 max-h-96 px-1 sm:px-4 w-full">
            {results.map((video) => (
              ...
              >
                <img src={video.thumbnail} alt={video.title} className="w-12 h-9 sm:w-16 sm:h-12 rounded object-cover shrink-0" />
                <div className="flex-grow min-w-0">
                  <p className="font-semibold text-xs sm:text-sm  text-foreground" title={video.title}>
                    {video.title}
                  </p>
