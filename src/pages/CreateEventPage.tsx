  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    while (!eventCreated && attempts < 5) {
      ...
      const { data, error } = await supabase
        .from('events')
        .insert({ name: eventName, access_code: newAccessCode })
        .select()
        .single();
      ...
      <div className="container mx-auto p-4 flex flex-col items-center justify-center min-h-screen">
        <Card className="w-full max-w-md bg-spotify-light-dark text-spotify-text">
          ...
          <CardContent className="space-y-4">
            <div>
              <p className="text-sm text-spotify-text-muted">Nombre del Evento:</p>
              <p className="text-lg font-semibold">{eventName}</p>
            </div>
            <div>
              <p className="text-sm text-spotify-text-muted">Código de Acceso:</p>
              <p className="text-3xl font-bold text-spotify-green">{accessCode}</p>
            </div>
          </CardContent>
          <CardFooter className="flex flex-col gap-2">
            <Button onClick={() => navigate(`/event/${eventId}`)} className="w-full bg-spotify-green hover:bg-spotify-green/90 text-spotify-dark font-semibold">
              Ir al Evento
            </Button>
    ...
    <div className="container mx-auto p-4 flex flex-col items-center justify-center min-h-screen">
      <Card className="w-full max-w-md bg-spotify-light-dark text-spotify-text">
        <CardHeader>
          ...
          <CardDescription>Dale un nombre a tu evento para empezar.</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            <div className="space-y-1">
              <Label htmlFor="eventName" className="text-spotify-text-muted">Nombre del Evento</Label>
              <Input
                id="eventName"
                type="text"
                value={eventName}
          ...
          <CardFooter>
            <Button type="submit" disabled={isLoading} className="w-full bg-spotify-green hover:bg-spotify-green/90 text-spotify-dark font-semibold">
